...
0. **Confirm you are running inside the AI-Training repo (the source of truth).** Verify that `{project-root}/.claude/skills/bmad-elcai-story-loop/SKILL.md` exists. If it does not, STOP and tell the user:
   ...
   > This command must be run from inside the `AI-Training` repo (the source of the story-loop skill). Open Claude Code in the AI-Training checkout and try again.
...
2. **Preflight — BMAD 6.10.x must be installed in the target.** The bmad-elcai-story-loop skill wraps `bmad-dev-auto` (and optionally invokes `bmad-create-story` when the user picks option 3 at runtime). Check ALL of these exist:
   ...
   >
   > The story-loop wraps skills shipped by `bmad-method` — they must be present before the loop can run. Install (or update) BMAD, then try `/install-story-loop` again:
   >
   > ```
   > npx bmad-method@6.10.1-next.12 install --directory {target} --action update --yes
   > ```
   >
   > Missing files:
...
3. **Show the user what will be copied and ask for approval.** Present this batch, then ask "Proceed?":
   ...
   - `.claude/commands/install-story-loop.md` → `{target}/.claude/commands/install-story-loop.md` (so the target can re-install elsewhere)

   Files to modify (best-effort):
   - `{target}/.gitignore` — add allowlist lines for `.claude/skills/bmad-elcai-story-loop/**` (only if the target's `.gitignore` already blanket-ignores `.claude/*`)
...
5. **Copy this install command shim** (so the target project can install to further projects). Create `{target}/.claude/commands/` if missing, then copy `{project-root}/.claude/commands/install-story-loop.md` to `{target}/.claude/commands/install-story-loop.md`.
...
6. **Update the target `.gitignore` (best effort).** Read `{target}/.gitignore` if it exists. If it contains any `.claude/*` blanket-ignore line but does NOT already contain `!.claude/skills/bmad-elcai-story-loop/`, append this block to the bottom of the file:
   ...
   ```
   # Allowlist bmad-elcai-story-loop skill + install shim so participants get them via git pull
   !.claude/commands/
   .claude/commands/*
   !.claude/commands/install-story-loop.md
   !.claude/skills/
   .claude/skills/*
...
7. **Verify.** After copying, check that:
   ...
   - The 4 preflight files listed in Step 2 STILL exist (BMAD wasn't disturbed).
   - `install-story-loop.md` exists at `{target}/.claude/commands/`.

8. **Report the outcome to the user:**
   - List of files copied (with target paths).
   - `.gitignore` state (appended / unchanged / absent).
   - Next step: "Restart Claude Code in `{target}`. The `bmad-elcai-story-loop` skill should appear in the skill list. Invoke it by saying 'run the story loop' or by explicit skill activation."
