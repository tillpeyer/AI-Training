...
if ($cmd -match 'git\s+add' -and $cmd -notmatch '\-\-dry-run') {
    $leaked = git status --porcelain --ignored=no --untracked-files=all -- .agents _bmad .claude 2>$null |
        Where-Object { $_ -match '^\?\? (\.agents/|_bmad/(?!custom/config\.toml)|\.claude/(?!skills/install-story-loop/|skills/bmad-elcai-story-loop/|hooks/(git-safety-guard\.(ps1|sh)|notify\.sh)$|settings\.json$|commands/(elcai-check-env|check-training-env)\.md$))' }
    if ($leaked) {
